...
from shop import ShopRepository, Plugin
...
class ShopCallbackHandler(CallbackHandler):
    ...
    async def handle(self, callback, bot):
        ...
        await bot.edit_message(callback.from_user_id, callback.message_id, text, parse_mode='HTML')
...
class BuyCallbackHandler(CallbackHandler):
    async def handle(self, callback, bot):
        plugin = self.shop.get_plugin(callback.data[4:])
        # TODO: Implement BuyCallbackHandler later
